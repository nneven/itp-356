...
contract ComeFundMeV2 is Ownable, Pausable {
    struct Campaign {
        bool isAlive;
        address initiator;
        uint256 fundsRaised;
    ...
    function endCampaign(bytes32 campaignId) external whenNotPaused {
        Campaign storage c = _campaigns[campaignId];
        require(c.deadline < block.number, "Not past deadline");
    ...
    {
        Campaign storage c = _campaigns[campaignId];
        require(c.deadline >= block.number, "Deadline has passed");
        require(c.isAlive, "Already ended");
    ...
    /**
        @notice Withdraws ETH from a certain campaign if deadline has not passed
        @param campaignId The ID of the campaign that we are trying to withdraw from
        @param amount The amount to withdraw from the campaign to the user
     */
    function withdrawFromCampaign(bytes32 campaignId, uint256 amount)
        external
        whenNotPaused
    {
        Campaign storage c = _campaigns[campaignId];
